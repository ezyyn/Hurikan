...
void Player::Init(Scene* scene, GameGrid* gamegrid)
{
	...
	m_PlayerEntity.AddCustomComponent<PlayerStats>();
	m_PlayerEntity.AddComponent<SpriteRendererComponent>(glm::vec4(1.0f));

	m_FrameAnimator = &m_PlayerEntity.AddCustomComponent<FrameAnimator>(m_PlayerEntity);

	if (!m_Initialized)
	{
		m_Initialized = true;

		m_BombAnimationSpriteSheet = Texture2D::Create("assets/textures/bomb_animation/ba_128x128.png");
		SpriteSheet = Texture2D::Create("assets/textures/player_animation/Chr_00_0.png");
		SpriteSheet1 = Texture2D::Create("assets/textures/tileset_16x16.png");
		...
		{
			m_PlayerDead.Tag = "PlayerDead";
			m_PlayerDead.SpriteSize = { 16,16 };
			m_PlayerDead.SpriteSheet = SpriteSheet;
			std::array<glm::vec2, 7> positions;
			std::array<float, 7> delays = { 300.0f, 300.0f, 300.0f, 300.0f, 300.0f, 300.0f, 300.0f };
			...
			m_PlayerDead.Animation.reserve(positions.size() + 1);
			for (size_t i = 0; i < positions.size(); i++)
			{
				FrameData frameData;
				frameData.Subtexture = SubTexture2D::CreateFromCoords(m_PlayerDead.SpriteSheet, positions[i], m_PlayerLeftAnimation.SpriteSize);
				frameData.Delay = delays[i];

				m_PlayerDead.Animation.emplace_back(frameData);
			}
...
#define BOMB_EXPLOSION_CENTER_SPREAD_ANIMATION_SIZE 7
	...
	m_FrameAnimator->Add(m_PlayerLeftAnimation);
	m_FrameAnimator->Add(m_PlayerUpAnimation);
	m_FrameAnimator->Add(m_PlayerDownAnimation);
	m_FrameAnimator->Add(m_PlayerDead);
	...
	g_GameGrid->ForEach_1([&m_PlayerEntity = m_PlayerEntity](Entity entity)
		{
...
void Player::OnUpdate(Timestep ts)
{
	...
	{
		bool exploded = m_PlacedBombs[i].Tick(ts);
		if (exploded) // If true bomb has exploded and vanished
			m_PlacedBombs.erase(m_PlacedBombs.begin() + i);
	}

	AudioManager::CheckStates();
}
...
void Player::PlaceBomb()
{
	if (CanCreateAnother())
	{
		// TODO: FIX MEMORY LEAK, REWRITE THIS GARBAGE
		AudioManager::Play(AudioType::BOMB_PLACED);
...
{
	if (m_PlacedBombs.size() >= stats().MaxBombCount)
		return false;
...
void Player::OnFatalHit()
{
	AudioManager::Play(AudioType::PLAYER_DEATH);
	m_FrameAnimator->Switch("PlayerDead");
}
